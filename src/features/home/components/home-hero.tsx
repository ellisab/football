        ...
        <div className="relative z-10 mx-auto flex min-h-[calc(100svh-6.5rem)] w-full max-w-[1240px] flex-col px-4 pb-20 pt-8 sm:px-6 sm:pb-24 sm:pt-10 lg:px-10">
          <div className="grid flex-1 items-center gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div className="mx-auto grid max-w-[22rem] gap-4 text-center sm:mx-0 sm:max-w-[45rem] sm:text-left">
              ...
              <h1 className="text-[2.75rem] leading-[0.86] font-[var(--font-stadium-heading)] uppercase tracking-[0.03em] text-[#f5edc9] [text-shadow:0_0_30px_rgba(2,9,12,0.36)] sm:max-w-[12ch] sm:text-[4.4rem] lg:text-[5.4rem]">
                {headline}
              </h1>
              ...
              <div className="flex flex-wrap justify-center gap-3 sm:justify-start">
                <Link
                  href={primaryHref}
                  className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(94deg,#f5edc9_0%,#d8b86a_46%,#ffb45f_100%)] px-5 py-3 text-sm font-semibold text-[#030708] shadow-[0_20px_45px_rgba(8,17,22,0.36)] transition-transform duration-300 hover:-translate-y-0.5"
                >
                  Zu den Spielen
                ...
                {showSecondaryAction ? (
                  <Link
                    href={secondaryHref}
                    className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.06] px-5 py-3 text-sm font-semibold text-[#f2f7f2] backdrop-blur-md transition-colors duration-300 hover:bg-white/[0.11]"
                  >
                    Zur Tabelle
                  </Link>
                ) : null}
            ...
            <aside
              className="poster-surface mx-auto grid w-full max-w-[23rem] gap-2 rounded-[1.35rem] border border-white/10 bg-[#061512]/66 p-3 shadow-[0_18px_42px_rgba(2,9,12,0.28)] backdrop-blur-md sm:grid-cols-3 lg:mx-0 lg:grid-cols-1"
              aria-label="Kurzueberblick"
            >
              <div className="flex items-center gap-2 px-2 py-1 sm:col-span-3 lg:col-span-1">
                <Trophy className="h-4 w-4 text-[#d8b86a]" />
                <span className="text-[0.68rem] font-bold uppercase tracking-[0.18em] text-[#8fa59c]">
                  Spieltag-Zentrale
                </span>
              </div>
              {previewStats.map((stat) => (
                ...
                >
                  <div className="truncate text-[0.62rem] font-bold uppercase tracking-[0.14em] text-[#8fa59c]">
                    {stat.label}
                  </div>
                  <div className="mt-1 truncate font-mono text-2xl font-bold text-[#f5edc9]">
                    {stat.value}
                  </div>
                  {stat.detail ? (
                    <div className="mt-0.5 truncate text-[0.68rem] text-[#afc4bb] sm:text-xs">
                      {stat.detail}
                    </div>
                  ) : null}
                </div>

  ...
  return (
    <section className="grid gap-5">
      <div className="grid gap-2">
        <h2 className="text-[1.85rem] leading-[0.9] font-[var(--font-stadium-heading)] uppercase text-[#f5edc9] sm:text-[2.2rem]">
          <span className="inline-flex items-center gap-2">
            <Trophy className="h-6 w-6 text-[#d8b86a]" />
            {title}
          </span>
        </h2>
      ...
      {rounds.map(({ group, matches }) => {
        ...
        return (
          <div key={group.groupID ?? group.groupName} className="grid gap-3">
            <div className="text-[0.7rem] font-semibold uppercase tracking-[0.24em] text-[#6eeaf2]">
